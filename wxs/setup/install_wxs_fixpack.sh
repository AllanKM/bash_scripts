...
#---------------------------------------------------------------
# WXS fixpack installer
#---------------------------------------------------------------

# USAGE: install_wxs_fixpack.sh fix=<fixpackid> [was=<61|70>]
#  Note: Omitting was=<version> will assume a stand-alone WXS fixpack install
unset JAVA_HOME
unset WAS_HOME
...
until [ -z "$1" ] ; do
    case $1 in
		was=*)  VALUE=${1#*=}; if [ "$VALUE" != "" ]; then WASVERSION=$VALUE; fi ;;
		fix=*)  VALUE=${1#*=}; if [ "$VALUE" != "" ]; then FIXPACKID=$VALUE; fi ;;
		*)	echo "#### Unknown argument: $1"
			echo "#### Usage: install_wxs_fixpack.sh fix=<fixpackid> [was=<61|70>]"
			echo "####  Note: Omitting was=<version> will assume a stand-alone WXS fixpack install"
			exit 1
			;;
	esac
	shift
done
if [ -n "$WASVERSION" ]; then
	WASVER=`echo $WASVERSION | cut -c1-2`
	...
	UPDIRESPONSE="/lfs/system/tools/was/responsefiles/v${WASVER}silent.updi.script"
else
	WXSVER=`echo $FIXPACKID | cut -c1,3`
	BASEDIR="/usr/WebSphere"
	if [ -d ${BASEDIR}/eXtremeScale${WXSVER} ]; then
	    APPDIR="${BASEDIR}/eXtremeScale${WXSVER}"
    else
	    echo "Unable to determine base application server path!"
	    exit 1
    fi
	UPDIRESPONSE="/lfs/system/tools/wxs/responsefiles/wxs.silent.updi.script"
fi
...
wxs_fixpack ()
{
    ...
    PRODFILE="${APPDIR}/properties/version/WXS.product"
    ...
    if [[ ! -d $UPDDIR || $CURFIXLEVEL -lt $NEWFIXLEVEL ]]; then
        ...
        ./install -silent -options "${UPDIRESPONSE}"
	...
	if [ $? -eq 0 ]; then
		...
		echo ${FIXPACKID} | grep -q cumulative
		if [ $? -eq 0 ]; then
			FIXTYPE=IF
		else
	...
	if [ -n "$WASVERSION" ]; then
		...
		REPLACE_OBJECTGRID_HOME=${APPDIR}/ObjectGrid
		. ${APPDIR}/ObjectGrid/bin/setupCmdLine.sh
	fi
    cd ${UPDDIR}
...
WXSDOTVER=`echo ${FIXPACKID} | cut -c1-3`
FIXDIR="/fs/system/images/websphere/wxs/${WXSDOTVER}/fixes"
JAVAVER=`${APPDIR}/java/jre/bin/java -version  2>&1 |grep '^IBM J9 VM'`
