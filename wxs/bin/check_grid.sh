...
# set client auth settings for curl if necessary
if [ "$PROTO" == "https" ]; then
        wasKeyfile=/lfs/system/tools/was/etc/ei.gz.was.client.pem
        if [ -r /lfs/system/tools/was/etc/was_passwd ]; then
                while read line; do
                        if [[ "$line" = "clientauth_gz="* ]]; then
                                wasBase64pw=${line#*=}
                        fi 
        ...
        wasPW=$(print $wasBase64pw | /usr/local/bin/perl -ne 'use MIME::Base64; print decode_base64($_);')
        clientAuth="-L --cert $wasKeyfile --pass $wasPW"
else
        clientAuth=""
fi
...
#	print -u2 -- "curl --connect-timeout $timeout  -sI -H\"Host: $HOST\" $PROTO://$sampleNode:$PORT$URI"
	cookie=`curl --connect-timeout $timeout -sI $clientAuth -H"Host: $HOST" $PROTO://$sampleNode:$PORT$URI | awk '/Set-Cookie/ {print $2}'`
	wasCookie=`echo $cookie| awk '{split($0,c,";"); print c[1]}'`
	wxsCookies=`echo $cookie| awk '{split($0,c,";"); print c[2]";"c[3]}'`
  
	if [ -n "${wasCookie}" ] ; then 
		wasCookie=${wasCookie%;} #remove trailing semi-colon
		wasCookie=${wasCookie%%:*} #remove host_identifiers
		wasCookiename=${wasCookie%%=*} #save the cookiename (removing everything right of the equal sign)
		session=${wasCookie##*=????} #save the session (removing the cookiename and the counter)

		print --  "Established session \"${wasCookiename##SESSION_}\" with value \"$session\" on \"$sampleNode\" through \"$myURL\" "
		...
		for node in $nodes; do 
			 (
				curl --connect-timeout $timeout -m $timeout -s $clientAuth -b "$wasCookie:`whoami`; $wxsCookies;" -H"Host: $HOST" -I $PROTO://$node:$PORT$URI -D "/tmp/${0##*/}.${node}.${myPID}.tmp" >/dev/null
				RC=$?
				case $RC in
				...
				*)	#print status=$RC
					if [ -s "/tmp/${0##*/}.${node}.${myPID}.tmp" ] ; then 
						if grep -q -- "$session" /tmp/${0##*/}.${node}.${myPID}.tmp 2>/dev/null; then
							print -- "	Session \"$session\" is valid on \"$node:$PORT\" for \"${wasCookiename##SESSION_}\". Curl RC=$RC."
						else
							print -u2 -- "#### $now:  $PROTO://$node:$PORT$URI cannot find session \"$session\" set by \"${sampleNode}:${PORT}\" in pool \"${wasCookiename##SESSION_}\". Curl RC=$RC." 
						fi
					fi
				esac
