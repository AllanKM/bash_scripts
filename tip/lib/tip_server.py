...
def usage():
        ...
        print '   - set log #backups, roll period:         -attr log_roll:14/24'
...
if (actionName == 'start'):
	...
	# Break up attr parameter to get the name
        ...
        if (attrName == 'log_roll'):
        	# Log rotation and retention based in TIME   MEC
                maxBackups = attrValues.split('/')[0]
                rollPeriod = attrValues.split('/')[1]
                serverNameID = AdminConfig.getid('/Node:'+nodeName+'/Server:'+serverName+'/')
                # print '   serverNameID:'  +  str(serverNameID)
                print '   max backups: %s, roll period: %s' %(maxBackups, rollPeriod) 
                ## Set log file rotation/retention
                outlog = AdminConfig.showAttribute(serverNameID, 'outputStreamRedirect')
                errlog = AdminConfig.showAttribute(serverNameID, 'errorStreamRedirect')
                try:      
                    AdminConfig.modify(outlog, [['rolloverType', 'TIME'], ['maxNumberOfBackupFiles', maxBackups], ['rolloverPeriod', rollPeriod], ['baseHour', 24]])
                    AdminConfig.modify(errlog, [['rolloverType', 'TIME'], ['maxNumberOfBackupFiles', maxBackups], ['rolloverPeriod', rollPeriod], ['baseHour', 24]])
                except:
                    print '### Error occurred while modifying log rollover and retention log_roll - exiting without save.'
                    print '### Error details:',sys.exc_info()
                    sys.exit()
                AdminConfig.save()
                print 'The configuration changes for log_roll have been saved.'
                print 'The appserver ('+serverName+') will need to be restarted.'
