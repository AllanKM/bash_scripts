...
#                   08-01-2014  NCO - support lookup for jks file name and pw                                    
#                   09-29-2014  NCO - Place $keyPassEnc inside quotes
#                   04-02-2015  New rules for keystore-PASSWD mapping
#                               Add new prompt-to-continue before ssl config
# 
...
USER=webinst 
GROUP=eiadm
# Indicate if user and group supplied at runtimeto overrid defaults
USER_OVERRIDE="n" 
GROUP_OVERRIDE="n"

# Override the EI-WAS supplied versions of the key and trust store when we must
# as is always the case with NCO. NCO does not use the EI standard certs. 
# 
# "DKTS"  =  Default Key Trust Store 
DKTS_OVERRIDE="n"    # y|n  Override to NodeDefaultTrustStore and KeyStore
DKTS_filename="-"    # "node.ini" indicatres we use etc/node_jks.ini 
#                    # for node-to0-jks-file mapping
DKTS_password="-"    # "pw.ini"   indicates we use etc/passwds.ini to getr the password
dkts_lookup_used=n
...
#Lookup real DKTS values
# In:  $1 - $DKTS_filename - a real name or "file.ini" 
#      $2 - $DKTS_password - a real pw   or "pw.ini"
#      $WAS_HOME  $PROFILE $TIP_TOOLS $NODE
# Out: $DKTS_filename and $DKTS_password - possibly updated  
lookup_DKTS_values() {
    local file pw keyStore keyPass keyPassEnc
    file=$1; pw=$2
    keyStore=$file   
    [ -n "$DEBUG" ] && echo "lookup_DKTS_values executing for $1, $2"  
    if [ "$file" == "file.ini"  -o "$file" == "node.ini"  ];  then
       # Locate the real DKTS file name from node_jks.ini
       [ -n "$DEBUG" ] && echo " Looking up  DKTS file name using $NODE"
       keyFile=$(grep $NODE $TIP_TOOLS/etc/node_jks.ini| cut -d':' -f2)
       dkts_lookup_used=y
       if [ "$keyFile" == "" ]; then 
       	   echo " DKTS keystore name not found for node $NODE...terminating"
       	   exit 1;
       fi	
       DKTS_filename=$keyFile
    fi
    
    if [ $pw == "pw.ini" ]; then
   	   # Locate encoded pw in lfs tools tip etc/password.ini
   	   [ -n "$DEBUG" ] && echo " Looking up encoded pw for $DKTS_filename in $TIP_TOOLS/etc/passwds.ini"
   	   # [ -n "$DEBUG" ] && echo "   in $TIP_TOOLS/etc/passwds.ini" 
       keyPassEnc=$(grep $DKTS_filename $TIP_TOOLS/etc/passwds.ini | cut -d':' -f2)
       dkts_lookup_used=y
       if [ "$keyPassEnc" == "" ]; then 
       	   echo "keystore enc pw not found for $DKTS_filename...terminating"
       	   exit 1
       fi	   
       DKTS_password=$($TIP_TOOLS/bin/Passwd_wrapper.sh decode  "$keyPassEnc"  $WAS_HOME)
   fi
   [ -n "$DEBUG" ] && echo " lookup result: $DKTS_filename $DKTS_password" 
...
########################################################
  ...
  #3. Type of TIP Using WASHOME  ( TCR, TCR31, TAD, NCO )
  determine_default_TIPID
  
  #4. Type of LDAP to configure if any . Default is blue(pages)  
  if   [ $TIPID == "NCO" ]; then
         LDAP_TYPE="none"
...
# Special input lookup needed for default node trust store DKTS? - for NCO  
# if overridsen, there better be file and pw "value"

if [ $DKTS_OVERRIDE == "y" ]; then
	 # confirm ?
   lookup_DKTS_values "$DKTS_filename" "$DKTS_password"   
fi 
...
#
  ...
  if [ $DKTS_OVERRIDE == "y" ]; then
    ...
    echo "..   using lookup:          $dkts_lookup_used"      
  fi
...
# Check for user
if [[ -z `id $USER` ]]; then
  echo "Error: user $USER does not exist...terminating"
  exit 1
fi
...
if [ $DKTS_OVERRIDE == "n" ]; then
  ...
  symlink=$keyStore
  if [ ! -e $file_path ]; then
    echo "*ERROR*  Override to KeyStore does not exist at: $file_path...terminating" 
    exit 1  
  else
...
if [[ -e $WAS_HOME/etc/$keyStore ]]; then
  ...
  ln -fs $WAS_HOME/etc/$keyStore $WAS_HOME/profiles/$PROFILE/etc/$symlink
	...
	echo "Error: Keystore $keyStore not found on node at $keyloc...terminating"
	exit 1
fi
...
DEC_PROF_NAME=""   # If TIPProfile, this variable not needed
#if [ $PROFILE == JazzSMProfile ]; then
...
if [ -e $WAS_PASSWD ]; then
	...
	# Revised 2015-04-03 to meet new PASSWD TO KEYSTOR mapping rules 
	#if [ "$zone" == "b" ]; then
	...
	echo "Error: WebSphere password setup file not found...terminating"
	exit 1
fi
...
if [[ -x $WSADMIN ]]; then
	...
	if [[ -e $WASLIB/sslConfig.py && -e $TIPLIB/tip_was_securityConfig.py ]];  then
		LOGGING="-tracefile /logs/was${VERSION}/${PROFILE}/wsadmin.sslConfig.$DATE.traceout"
	  ...
	  ## not used SSL_SCRIPT="$WASLIB/sslConfig.py"
	  ...
	  SSL_PY_ARGS="-node $WAS_NODE -keystore $keyStore -keypassword $keyPass -version $VERSION"
		...
		if [ $PROFILE == TIPProfile -o $PROFILE == JazzSMProfile  ]; then
		    ...
		    # The CONNARGS work below is not used. except for the ssl special so I will leave in
	  ...
	  #       -tracefile /logs/was70/TIPProfile/wsadmin.sslConfig.$DATE.traceout -f /lfs/system/tools/was/lib/sslConfig.py 
	  #       -node TIPNode -keystore z10031_netcool.jks -keypassword pw -version 70 
	 
	  # *NOTE* - We are not passing $CONNARGS here ! 
	  ...
	  # Our wsadmin scripts should be smart enough to determine if LMT store are defined and if so, configure them.
        ...
        #      ..result:  Success! Saving configuration...
	      ...
	      echo "  Return code: $ssl_rc "   
            ...
            prompt_to_continue
		        ...
		        #echo "  Return code: $? "   
		        #echo "..result: "  $result
		        # This style displays the jython script output messages in a more readable format
    ...
    if [[ -z $noServer ]]; then
      ...
      echo "         security call to be: $bypassed"
       ...
       LOGGING="-tracefile /logs/was${VERSION}/${PROFILE}/wsadmin.securityConfig.$DATE.traceout"
       [ -n "$DEBUG" ] && ADDARGS="$ADDARGS -debug"
   	   [ -n "$DEBUG" ] && echo  ldapPass=$ldapPass bindPass=$bindPass  keyPass=$keyPass
		   ...
		   #  WAS(standalone) on this host. We assume by default the userid/pw may not in soap.client.props 
		   #  and that the pw is tipadmin. The arg -tippw <pw> will override the default pw "tipadmin" 
		   #  The arg -useClientProps directs us to NOT pass userid/pw to wsadmin
		   CONNARGS=""
    ...
    # Bypass update userid/pw in soap.client.props ?
	    ...
	    # This code below moved  to it;s own section 
	  ...
	  # Update ssl.client props if ssl config was successful..  
	  # Input: $WAS_PROPS/ssl.client.props, $keyStore, $keyPassEnc 
	  if [ $SSL_CONFIG_SUCCESS == y ]; then
        echo "Updating ssl.client props.. because ssl config was successful"            	
        echo "   Using keyStore=$keyStore, keyPassEnc=$keyPassEnc " 
        echo "   $WAS_PROPS/ssl.client.props.$DATE --> $WAS_PROPS/ssl.client.props"
        echo "   will substitute location: $WAS_HOME/profiles/$PROFILE/etc/$keyStore"
        prompt_to_continue  
	      echo "    Creating backup in $WAS_PROPS/ssl.client.props.$DATE"
	      
	      su - $USER -c "cp -p $WAS_PROPS/ssl.client.props $WAS_PROPS/ssl.client.props.$DATE"
		    if [ $? -ne 0 ]; then echo "  backup failed...terminating"; exit 1; fi; 
		    echo "    Creating backup...completed, now update ssl.client.props "
		    su - $USER -c "sed -e \"s#Store=.*#Store=$WAS_HOME/profiles/$PROFILE/etc/$keyStore#g;s/StorePassword=.*/StorePassword=$keyPassEnc/g;s/Type=.*/Type=JKS/g\" $WAS_PROPS/ssl.client.props.$DATE > $WAS_PROPS/ssl.client.props"
       #su - $USER -c "sed -e \"s#Store=.*#Store=$WAS_HOME/profiles/$PROFILE/etc/$keyStore#g;s/StorePassword=.*/StorePassword=$keyPassEnc/g;s/Type=.*/Type=JKS/g\" $WAS_PROPS/ssl.client.props.$DATE > $WAS_PROPS/ssl.client.props"
        if [ $? -ne 0 ]; then	 echo "  update failed...terminating"; 	 exit 1; fi 
        test_scan=$(grep $keyStore $WAS_PROPS/ssl.client.props) 
        if [ "$test_scan" == "" ]; then echo "  update failed...terminating"; exit 1; fi
        echo "  Updating ssl.client.props..complete"
    else
