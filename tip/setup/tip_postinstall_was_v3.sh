...
# Usage: tip_postinstall_was_v3.sh VERSION [-washome $washome] [-user $user]
...
#           -shareclasses_none 
#           -modify_server_attrs|-server_attrs    - not supported ?
#           -jvm_msgs|-jm)     show jvm messages  - set in debug mode
#           -bypassSST|-bsst|-bs
#           -bypassLOGR|-blogr|-bl
#           -bypassXSR|-bxsr
#           -bypassHEAP|-bheap|-bh   bypass heap update even if -heap  specified
#           -debug
#
#   Description:
...
#      sudo  ./tip_postinstall_was_v3.sh 70  -washome $WASHOME -heap 512 1024 
#      sudo  ./tip_postinstall_was_v3.sh 85  -washome $WASHOME -heap 512 2048 
#      sudo  ./tip_postinstall_was_v3.sh 85  -wh $WH -pr $PR  -node $NODE  -heap 512 2048 -debug 
#
#   Change History: 
...
#     E Coley       10-09-2013  Update doc example on heap size
#                   06-10-2014  v3 Suppport jazz profile
...
if [ $BYPASS_SST == "n" ]; then
  ...
  echo "Checking status of $SERVER ...bypassed on request"  
fi
echo
#################################################
#
   ...
   if [[ -e $WASLIB/server.py ]]; then
     echo "Updating min/max heap sizes for $SERVER ...."
	   ...
	   [ -n "$DEBUG" ] && echo "  HEAP_CMD=\"$HEAP_CMD\""
 	   #xx su - $USER -c "$WSADMIN $LOGGING -f $WASLIB/server.py  -action modify -server $SERVER -attr $HEAP" $JVM_OUTPUT_DIRECTIVE
...
echo
echo "Installing the logrotate configuration for WAS logs at /logs/was__"
...
echo "Installing the logrotate configuration for WAS logs...complete"
