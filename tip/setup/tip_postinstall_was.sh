...
# Usage: tip_postinstall_was.sh VERSION [COREGROUP] [PROFILE]
#
#   Profile name not profile path.   
#   Change History: 
...
case $VERSION in
    ...
    WAS_CELL=$(grep WAS_CELL= $PROFSETUP|awk '{split($0,pwd,"WAS_CELL="); print pwd[2]}')
		...
		if [ "$DMCELL" != "" ] || [ -d /usr/WebSphere${VERSION}/AppServer/profiles/${PROFILE}/logs/dmgr ]; then
			...
			SERVER=server1
			HEAP=heap:128/256
			# Added for TIPProfile   
			if [ $PROFILE == "TIPProfile" ]; then
			    echo "  TIPProfile heap setting  512:1024"  
			    HEAP=heap:512/1024
			fi         
		else 
...
if [ "$SERVER" == "nodeagent" ]; then
	...
	if [[ $COREGROUP =~ ^p[123]_* ]] || [[ $COREGROUP =~ ^pre_* ]] || [[ $COREGROUP =~ ^cdt_* ]] ; then
