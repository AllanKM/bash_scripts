#!/usr/local/bin/perl
# Description: Validates URL is responding by using url_clientauth.pm
#
# Usage
#   chk_url_clientauth.pm [name] [url] [string]

# Branched from chk_url.pm to use url_clientauth.pm (which now uses curl in order to present the certificate/password for CA, as wget cannot provide cert password)
