...
foreach my $server ( sort @$serverlist ) {
	...
	disable( $dp_user, $server, $domain, $objects, 'MQQM' );
	disable( $dp_user, $server, $domain, $objects, 'HTTP.*Source.*' );
	...
	foreach my $object ( sort { lc($a) cmp lc($b) } keys %$objects ) {
		if ( $objects->{$object}->{class} =~ $class ) {
			if ( ( $name && $object =~ /$name/i ) || !$name ) {
				...
				setObjectState( $dp_user, $server, $domain, $objects->{$object}->{class}, $object, 'disabled' );
				print "\n";
			}
		}
	}
