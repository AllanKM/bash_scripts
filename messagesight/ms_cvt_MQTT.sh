...
while getopts p:v opt; do
  ...
  p)
    plex=$OPTARG
    if [[ $plex != "p1" && $plex != "P1" && $plex != "p2" && $plex != "P2" && $plex != "p3" && $plex != "P3" && $plex != "pre" && $plex != "PRE" && $plex != "ecc" && $plex != "ECC" ]] ; then
      echo "Must specify p1|p2|p3|pre|ecc"
      exit 2
    fi
    ;;
...
IMAPATH=$PWD
CLASSPATH=$PWD/com.ibm.micro.client.mqttv3.jar:$PWD/org.eclipse.paho.client.mqttv3.jar:$PWD
...
case $plex in
  ...
  ecc)
    SUBEPS1="9.42.180.227 9.42.180.145"
    PUBEPS1="9.42.180.227 9.42.180.145"
    ;;
...
test_publishing() {
  ...
  SUBCMD="$JAVA -classpath $CLASSPATH EIMQTTClient -a subscribe -t events/hpods/test/MSe2eprobe -s 0 -b $sub -p 8000 -u scoreboard -P PA16h9lD3J7F1d2 -c 1"
  ...
  PUBCMD="$JAVA -classpath $CLASSPATH EIMQTTClient -a publish -t events/hpods/test/MSe2eprobe -s 0 -b $pub -p 1500 -u r00t$ -P BF7r4AKs6X537aL -m \"$MESSAGE\""
