...
#
# Encodes the password provided into WebSphere's preferred {xor} encoded format.
#
...
if [ $# -eq 1 ]; then
	"$JAVA_HOME/bin/java" "-Dws.ext.dirs=$WAS_EXT_DIRS:$WAS_USER_DIRS" "-Djava.ext.dirs=$JAVA_EXT_DIRS" $JVM_EXTRA_CMD_ARGS -classpath "$WAS_CLASSPATH" com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.security.util.PasswordEncoder $1
else
	echo USAGE:  PasswordEncoder.sh password_to_encode
	exit
fi
