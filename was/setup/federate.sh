...
case $PROFARG in
        *anager)
			echo "ERROR: You cannot federate a DM to a DM."
			exit 1
			;;
        *)	if [ "$PROFARG" == "" ]; then
                PROFILE=$HOST
...
if [ "$COREGROUP" == "" ]; then
	ARGS="$DM $SOAPPORT -profileName $PROFILE -noagent"
else
	ARGS="$DM $SOAPPORT -profileName $PROFILE -coregroupname $COREGROUP -noagent"
fi
